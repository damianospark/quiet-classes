...
# 5모듈 | 통합 AI리터러시(AI활용 일반)

## 오리엔테이션

Duration: 5

4모듈에서 완성한 전략 기획 산출물(Docs·Sheets·Slides)을 Canva와 LLM을 활용해 **취업 포트폴리오**로 시각화합니다. 단일 `콘텐츠 담당자` 관점에서 디자인·설명력·제출까지 완성하는 2시간 실습입니다.
...
4. 4모듈 Notion 프로젝트 보드에 최종 산출물을 등록하고 담당 교수님께 제출한다.
...
| 단계                  | 도구                    | 활동                                      | 산출물            | 담당자        |
| ----------------------- | ------------------------- | ------------------------------------------- | ------------------- | --------------- |
| 1. 환경 준비          | Canva, Notion           | 4모듈 프로젝트 보드 확인, 5모듈 Task 등록     | 5모듈 Task 목록     | 콘텐츠 담당자 |
| 2. Canva 기본 기능    | Canva                   | 템플릿·브랜드 키트·레이아웃 가이드 실습 | 브랜드 프리셋     | 콘텐츠 담당자 |
| 3. 섹션 구성          | Canva                   | 4페이지 레이아웃, 4모듈 Docs·Sheets 임베드 | 와이어프레임      | 콘텐츠 담당자 |
...
1. ✅ [Canva 계정 로그인](https://www.canva.com/)하고 `홈 → 프로젝트`에서 `5모듈_포트폴리오` 폴더를 만든다.
2. ✅ Chrome에 Canva, [Gemini](https://gemini.google.com/), [ChatGPT](https://chatgpt.com/) 탭을 고정한다.
3. ✅ 4모듈에서 생성한 Notion `프로젝트 보드` 페이지를 열고 공유 링크를 확인한다.
4. ✅ 로컬 컴퓨터에 비정장 프로필 사진(정장 없이 촬영된 사진)을 준비한다.

### 5모듈 Task 등록

1. Notion `프로젝트 보드`에서 `+ 새 프로젝트`를 클릭하고 이름을 `5모듈 - 취업 포트폴리오 제작`으로 입력한다.
...
3. `프로젝트 보드`에서 `5모듈 - 취업 포트폴리오 제작` 행을 클릭하고 `Task Tracker`와 Relation을 맺는다.

### 성과물

- 5모듈 Task가 등록된 Notion 프로젝트 보드 스크린샷


![](img/20251109_172444_image.png)
...
4. ✅ 역량의 내용을 준비하기 어렵다면, 페이지 2에서 `요소 → 선 & 모양 → 프레임`을 드래그해 4모듈에서 만든 Google Docs 링크를 임베드한다.
   - 4모듈 Docs 링크를 복사하고 Canva 프레임에 붙여넣는다.
   - `임베드 → 링크 미리보기`를 선택해 문서 썸네일을 표시한다.
5. ✅ 성과의 내용을 준비하기 어렵다면 페이지 2 에서 `앱 → Google Drive → 연결`을 클릭하고 4모듈에서 만든 Google Sheet를 선택한다.
...
```
첨부한 이미지의 인물이 밝은 초록색 계열의 젊은 감성의 넥타이를 맨 모습으로 수정해줘 
```
...
```
아래의 내용을 기반으로 소개문 작성해
대상: 경영학 전공 3학년, AI 서비스 기획 직무 지원자.
목표: Canva 포트폴리오 첫 페이지에 들어갈 150자 내외 소개문 작성.
조건: 핵심 역량 3개(데이터 분석, 사용성 검증, 팀 협업)와 4모듈 프로젝트 성과(KPI 개선 25%)를 포함해줘.
톤: 신뢰감 있고 간결하게.
```
...
1. ✅ [ChatGPT Vision](https://chatgpt.com/) 또는 [Gemini Advanced](https://gemini.google.com/)를 열고 다음 중 하나를 업로드한다.
   - 4모듈 프로젝트 보드의 내용 ( PDF 또는 스크린 캡처)
   - GitHub 코드 ZIP (있을 경우)
2. ✅ 다음 프롬프트로 근거 기반 설명을 생성한다.

```
나는 컨테츠 담당인 역할 B를 맡아서 수행했어.
업로드한 파일을 기반으로, 포트폴리오 '프로젝트 소개' 섹션에 넣을 5문장 설명을 작성해줘.
...
4. ✅ 출력 텍스트를 Canva 페이지 2에서 `표` 또는 `리스트` 요소로 정리한다.
   - `도구 → 표 → 4×2`를 선택해 표를 만든다.
   - 열의 헤더는  `프로젝트명`, `결과`,  `사용 도구`, `나의 기여` 순으로 만든다.
...
1. ✅ Canva 페이지 2에서 `텍스트 → Magic Write`를 클릭하고 앞의 실습에서 사용했던 프로젝트 및 태스크 보드의 내용을 보고 프로젝트의 의미, 프로젝트로부터 나는 얼마나 성장했는 가를 묘사하는 한 개의 추가 섹션을 생성한다.
   - 예: "프로젝트 성과","프젝트를 통한 성장", etc

   ```
   첨부한 이미지에 있는 프로젝트 및 태스크 보드의 내용을 보고 프로젝트의 의미, 프로젝트로부터 나는 얼마나 성장했는 가를 묘사하는 내용을 담은 한 개의 추가 섹션을 알맞는 제목과 함께 생성해
   ```
...
3. ✅ 생성된 이미지를 가장 아래의 레이어로 세팅하고 투명도를 조정해서 배경처럼 두고 어울린다면 다음을 수행해 배경으로 설정한다.
   - 이미지를 드래그해서 원하는 페이지에 올려놓는다
   - 이미지를 마우스 오른쪽 버튼으로 클릭한 뒤 배경으로 설정 또는 배경 교체를 선택한다
   - 이 과정을 각 페이지마다 반복한다.
4. ✅ 인트로의 텍스트 영역을 선택한 뒤 영역위에 뜨는 툴바에서 `Magic Write`을 클릭하고 소개문을 영어·일본어 2개 언어로 번역하고 그 내용을 추가한다.
   - 번역된 텍스트를 인트로 영역에 추가하고 페이지의 레이아웃이나 색상 등을 재조정한다.
5. ✅ 배경 이미지를 선택한 다음 `Magic BG`버튼을 클릭하고 페이지의 배경이 자동으로 바뀌는 것을 확인한다.
6. ✅ 그 동안 수정하면서 기록된 과거의 버전들을 확인해본다.
...
![](img/20251109_165549_image.png)

</td>

<td width="50%">

![](img/20251109_165611_image.png)
...
2. ✅ `다운로드 → PDF 표준`을 선택해 저장한다.
3. ✅ Notion `Task tracker 보드`에서 `10. 포트폴리오 제작 (통합)` 행을 클릭하고 `Attach file` 속성에 Canva 링크와 PDF 파일을 업로드한다.
4. ✅ `상태`를 `완료`로 변경한다.
...
1. ✅ 소개문·프로젝트 설명에서 사용한 수치와 4모듈 KPI 값이 일치하는지 검토한다.
