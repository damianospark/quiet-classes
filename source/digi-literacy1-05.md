...
1. ✅ Chrome에 Canva, [Gemini](https://gemini.google.com/), [ChatGPT](https://chatgpt.com/) 탭을 고정한다.
1. ✅ 4강에서 생성한 Notion `프로젝트 보드` 페이지를 열고 공유 링크를 확인한다.
1. ✅ 로컬 컴퓨터에 비정장 프로필 사진(정장 없이 촬영된 사진)을 준비한다.
...
<보완필요: Canva 브랜드 키트 설정 화면 캡처>

## 포트폴리오 섹션 구성
Duration: 18

1. ✅ `페이지 추가`를 클릭해 총 4페이지를 구성한다.
   - **페이지 1**: 인트로 (프로필 사진 + 소개문)
   - **페이지 2**: 역량 (4강 Docs 기획 문서 요약)
   - **페이지 3**: 성과 (4강 Sheets KPI 차트)
   - **페이지 4**: 컨택 포인트 (이메일, GitHub, LinkedIn)
2. ✅ 각 페이지에서 `스타일 → 모든 페이지에 적용`을 클릭해 브랜드 컬러·폰트를 일괄 적용한다.
3. ✅ 페이지 2에서 `요소 → 선 & 모양 → 프레임`을 드래그해 4강 Docs 링크를 임베드한다.
   - 4강 Docs 링크를 복사하고 Canva 프레임에 붙여넣는다.
   - `임베드 → 링크 미리보기`를 선택해 문서 썸네일을 표시한다.
4. ✅ 페이지 3에서 `앱 → Google Drive → 연결`을 클릭하고 4강 Sheets를 선택한다.
   - `삽입 → 차트 → 시트에서`를 클릭하고 KPI 차트를 가져온다.
   - 차트 크기를 조정하고 `사용자 지정 → 범례`에서 글씨체를 브랜드 폰트로 변경한다.
5. ✅ 페이지 4에서 `텍스트 → 본문 추가`를 클릭하고 이메일·GitHub·LinkedIn 링크를 입력한다.
...
<aside class="info">
<strong>증명사진이 없다면?</strong> 정장 없이 촬영한 사진이나 넥타이 없이 찍은 사진을 Gemini nanobannana 또는 Qwen Image Edit로 리터치해 포트폴리오용 프로필 사진으로 활용할 수 있습니다.
</aside>
